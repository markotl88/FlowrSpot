...
private extension Mappable {
  static func parseGeneric<T: Mappable>(_ json: Any?, key: String?=nil) -> T? {
    guard let json = json as? RestClient.JSON else { return nil }
    
    var mappedObj: T?
    if let key = key, let nestedJson = json[key] as? RestClient.JSON {
  ...
  static func parseArrayGeneric<T: Mappable>(_ json: Any, key: String?=nil) -> [T] {
    ...
    if let key = key, let jsonArray = json as? RestClient.JSON {
      var currentNestedJson: [ String: Any ] = jsonArray
      let keys = key.components(separatedBy: ".")
      for nestedKey in keys.dropLast() {
        guard let nestedJson = currentNestedJson[nestedKey] as? RestClient.JSON else { return parseDirectlyHandler(json) }
